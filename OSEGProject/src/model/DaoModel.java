...
import controller.Account_Registration;
import controller.AdminPageController;
import controller.FGServices;
import controller.PServices;
	...
	public DaoModel() { 
		//create db object instance
		 conn = new DBConnect();
			 ...
			 */
			public String[] retrieveUserRecords(String emailOrUsername) {
				 ResultSet rs1 = null;
				 ...
				 String sql = "SELECT user_name,password,type from oseg_user where email='"+emailOrUsername+"'";
				...
				// execute the query
				 ...
				 if(rs1.next()) {
					 ...
					 values[0] = rs1.getString("user_name");
					 values[1] = rs1.getString("password");
					 values[2] = rs1.getString("type");
			...
			public List<FGServices> retrieveFGUrls() {
				 ResultSet rs1 = null;
				// String[] values = new String[2];
				 ...
				 stmt = conn.connect().createStatement();
				 System.out.println("Retrieving Records from the table...");
				 String sql = "SELECT service_type,service_name,url from oseg_url where service_type='Government'";
				 System.out.println(sql);
				// execute the query
				 rs1 = stmt.executeQuery(sql);
				while(rs1.next()) {
					fgService=new FGServices();
					fgService.setServiceName(rs1.getString("service_name"));
					fgService.setServiceType(rs1.getString("service_type"));
					fgService.setUrl(new Hyperlink(rs1.getString("url")));
					fgServiceList.add(fgService);
				}
				...
				return fgServiceList;
				
			}
			
			public List<PServices> retrievePUrls() {
				 ResultSet rs1 = null;
				// String[] values = new String[2];
				 try {
				 //open a connection
				 stmt = conn.connect().createStatement();
				 System.out.println("Retrieving Records from the table...");
				 String sql = "SELECT service_type,service_name,url from oseg_url where service_type='Private'";
				 System.out.println(sql);
				// execute the query
				 rs1 = stmt.executeQuery(sql);
				while(rs1.next()) {
					pService=new PServices();
					pService.setServiceName(rs1.getString("service_name"));
					pService.setServiceType(rs1.getString("service_type"));
					pService.setUrl(new Hyperlink(rs1.getString("url")));
					pServiceList.add(pService);
				}
			...
			public boolean insertRecords(List<Account_Registration> aList ) throws Exception
			{
				int success=0;
				try
				{
					...
					{
						sql = "insert into oseg_user(user_name,email,password,type) " +
								"values ('"+aList.get(i).getName()+"','"+aList.get(i).getEmail()+"','"+aList.get(i).getPassWord()+"','"+aList.get(i).getTypeOfUser()+"')";
				...
				catch(SQLException e)
				{
					...
					if(e.getMessage().contains("Duplicate")) {
						Alert alert = new Alert(AlertType.ERROR);
			  			...
			  			alert.setHeaderText("Duplicate Email ID");
			  			alert.setContentText("Email ID already exists");	
			  			alert.showAndWait();
					}
			...
			public boolean insertUrlData(List<AdminPageController> aList ) throws Exception
			{
				int ss=0;
				if(aList.size()>0)
				{
				...
				try
				{
					...
					for(int i=0;i<aList.size();i++)
					{
						...
						sql = "insert into oseg_url(service_type,service_name,url) " +
								"values ('"+aList.get(i).getServiceType()+"','"+aList.get(i).getServiceName()+"','"+aList.get(i).getTextAreaData()+"')";
